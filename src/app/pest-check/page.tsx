  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <main className="flex-1 p-4 md:p-6 lg:p-8 max-w-4xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <div className="md:col-span-5 flex flex-col gap-4">
            <Card className="border-primary/10 shadow-lg">
              ...
              <CardContent>
                <div className="border-2 border-dashed border-primary/10 rounded-2xl flex flex-col items-center justify-center p-4 h-[240px] bg-primary/[0.01] hover:bg-primary/[0.02] relative overflow-hidden transition-colors">
                  {leafImage ? (
                    <>
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                      <img src={leafImage} alt="Crop Leaf" className="absolute inset-0 w-full h-full object-cover" />
                      <div className="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                        <Label htmlFor="leaf-upload" className="cursor-pointer bg-white text-primary px-4 py-2 rounded-xl text-xs font-bold shadow-lg flex items-center gap-1.5">
                    ...
                    <div className="text-center space-y-3">
                      ...
                      <Label htmlFor="leaf-upload" className="cursor-pointer inline-flex bg-primary hover:bg-primary/90 text-white px-4 py-2 rounded-xl text-xs font-bold shadow transition-colors">
                        Choose File
                      </Label>
                    </div>
                  )}
          ...
          <div className="md:col-span-7 flex flex-col gap-4">
            <Card className="border-primary/10 shadow-lg flex-1">
              <CardHeader className="pb-3">
              ...
              <CardContent className="space-y-4">
                ...
                <Alert className="bg-primary/[0.02] border-primary/10">
                  ...
                  <AlertTitle className="text-xs font-bold text-primary dark:text-primary">Diagnosis Instructions</AlertTitle>
        ...
        {result && (
          <Card className="border-primary/20 bg-primary/[0.02] shadow-xl p-6 space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start gap-4 border-b border-primary/10 pb-4">
              ...
              <div className="flex items-center gap-2 self-end sm:self-auto">
                <Badge className={`text-xs uppercase font-mono px-3 py-1 ${
                  result.category === 'none' ? 'bg-primary' : result.category === 'nutrient_deficiency' ? 'bg-blue-600' : 'bg-red-600'
                }`}>
                ...
                <Button variant="outline" size="sm" onClick={handleTextToSpeech} className="border-primary/10 text-primary">
                  {playing ? <Loader2 className="h-4 w-4 animate-spin text-primary" /> : <Volume2 className="h-4 w-4" />}
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-primary/10">
                <h4 className="font-bold text-sm uppercase tracking-wider flex items-center gap-1.5">
                  <Calendar className="h-4 w-4 text-primary" /> Long-Term Prevention
                </h4>
